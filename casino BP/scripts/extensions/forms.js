...
class ChestFormData {
	...
	button(slot, itemName, itemDesc, texture, stackSize = 1, enchanted = false) {
		...
		var stackText = `stack#00001`;
		if (stackSize != 0) {
			stackText = `stack#${Math.min(Math.max(stackSize, 1) || 1, 50000).toString().padStart(5, '0')}`;
		} 
		this.#buttonArray.splice(slot, 1, [`${stackText}§r${itemName ?? ''}§r${itemDesc?.length ? `\n§r${itemDesc.join('\n§r')}` : ''}`,
		...
		for (let i = 0; i < pattern.length; i++) {
			...
			for (let j = 0; j < row.length; j++) {
				...
				if (key[letter]) {
					...
					var stackText = `stack#00001`;
					this.#buttonArray.splice(slot, 1, [`${stackText}§r${data?.itemName ?? ''}§r${data?.itemDesc?.length ? `\n§r${data?.itemDesc.join('\n§r')}` : ''}`,
