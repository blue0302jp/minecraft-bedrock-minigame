...
const MAX_DIST = 1.2;
const power = 0.13;
const punchPower = 0.22;
const EPS = 1e-4;                  // 0.0001
const SIDE_OFF        = 0.997;   // ヒットボックス半幅くらい
const SIDE_OFF2        = 0.5;   // ヒットボックス半幅くらい
const NORMAL_RAY_DIST = 1.6;
const SHORT_RAY_DIST  = 1.1;
const MAX_REFLECT_DIST = 1.46; // 反射判定の最大距離（必要に応じ調整）
const SIDE_REFLECT_DIST = 1.04;  // 真横ray用の反射判定距離


//サッカーボールが殴られたときの処理
export function rotateSoccerBallHandle(entity, player) {
    const playerRotation = player.getRotation();
    const dir = player.getViewDirection();
    entity.applyImpulse({ x: dir.x * punchPower, y: 0.65, z: dir.z * punchPower});
    entity.setRotation(playerRotation);

}

//サッカーボールがバウンドする処理
export function getSoccerBallBouncePower(entity) {
    if (!entity.isOnGround) return -0.016;
    const bounceCount = entity.getDynamicProperty("bounce");
    if (bounceCount > 0) {
        entity.setDynamicProperty("bounce", bounceCount - 1);
    }
    switch(bounceCount) {
        case 3:
            return 0.8;
...
export function soccerBallMovementHandle(entity, tick) {
    const dir = entity.getViewDirection();

    entity.applyImpulse({ x: dir.x * power, y: getSoccerBallBouncePower(entity), z: dir.z * power });

}
/*function getPower(tick) {
    let div;
    const value = tick / 20;
    if (value > 5) {
        div = 0;
...
export function soccerBallReflection(entity) {
...
function checkRayObstacle(dim, entity, origin, direction, distance) {
    ...
    return (entity.typeId === "soccer:soccer_ball") ? {hit : firstHit, isFirstHit : true} : null;
}
